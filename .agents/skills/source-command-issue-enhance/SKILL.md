---
name: "source-command-issue-enhance"
description: "Issue内容を分析し、ユーザーに質問しながら不足情報を補完・更新"
---

# source-command-issue-enhance

Use this skill when the user asks to run the migrated source command `issue-enhance`.
...
Anvilのモジュール構成を参考にする：
- `src/agent/` - エージェントループ・プロトコル
- `src/app/` - アプリケーションオーケストレータ
- `src/config/` - 設定管理
- `src/contracts/` - 共通型定義
- `src/extensions/` - スラッシュコマンド・拡張
- `src/provider/` - LLMプロバイダー（Ollama/OpenAI互換）
- `src/tooling/` - ツール実行・検証
- `src/tui/` - TUI描画
...
Q2（解決策が不足時）:
  ...
  options:
    ...
    - label: "{コードベース調査から推測される方針2}"
      description: "{詳細}"
    - label: "おまかせ"
      description: "コードベース調査結果を元にCodexが提案"
...
更新URL: https://github.com/Kewton/Anvil/issues/{issue_number}
