---
name: "source-command-pm-auto-dev"
description: "Issue開発を完全自動化（TDD→テスト→リファクタリング→報告）"
---

# source-command-pm-auto-dev

Use this skill when the user asks to run the migrated source command `pm-auto-dev`.
...
| エージェント | モデル/ツール | 理由 |
|-------------|-------------|------|
| tdd-impl-agent | **Codex opus** | コード生成にOpus必要 |
| **codex-reviewer** | **Codex**（commandmatedev経由） | 潜在バグ・セキュリティ脆弱性の独立レビュー |
| acceptance-test-agent | **Codex opus** | テスト品質にOpus必要 |
| refactoring-agent | **Codex opus** | コード改善にOpus必要 |
...
Codex（実装者）とは異なるモデルの視点で、潜在バグとセキュリティ脆弱性を検出します。
