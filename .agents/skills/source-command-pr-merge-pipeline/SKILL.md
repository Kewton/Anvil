---
name: "source-command-pr-merge-pipeline"
description: "複数IssueのPR作成→CI通過→順次マージ→統合検証を自動化"
---

# source-command-pr-merge-pipeline

Use this skill when the user asks to run the migrated source command `pr-merge-pipeline`.
...
for issue_num in {issue_numbers}; do
  gh pr list --repo Kewton/Anvil --head "feature/issue-${issue_num}" --json number,title,state
done
```
...
```bash
for issue_num in {issue_numbers}; do
  PR_NUM=$(gh pr list --repo Kewton/Anvil --head "feature/issue-${issue_num}" --json number -q '.[0].number')
...
```bash
for each PR:
  gh pr checks <PR_NUM> --repo Kewton/Anvil
```
...
```bash
for each PR:
  gh pr checks <PR_NUM> --repo Kewton/Anvil --watch
```
...
1. **各PRの変更規模を取得**:
   ```bash
   for each PR:
     gh pr diff <PR_NUM> --repo Kewton/Anvil --stat
   ```
...
5. マージ順序をユーザーに報告し確認を取る：
   ```
   マージ順序: #24 → #25
   理由: #24 の変更規模が大きく、agentic.rs の変更が多いため先にマージ
   この順序で進めてよろしいですか？
   ```
...
```bash
gh pr view <PR_NUM> --repo Kewton/Anvil --json mergeable,mergeStateStatus
```
...
commandmatedev send <worktree-id> \
  ...
  解消後:
  1. cargo build && cargo clippy --all-targets && cargo test で確認
...
```bash
gh pr merge <PR_NUM> --merge --repo Kewton/Anvil
```
