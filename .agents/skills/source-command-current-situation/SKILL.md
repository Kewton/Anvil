...
Use this skill when the user asks to run the migrated source command `current-situation`.
...
```bash
# 直近のAnvil実行ログがあれば取得
ls -la .anvil/logs/ 2>/dev/null | tail -5

# セッション情報
ls -la .anvil/sessions/ 2>/dev/null | tail -5
```
...
```bash
# 類似のオープンIssueがないか確認
gh issue list --repo Kewton/Anvil --state open --json number,title --limit 30
```
...
```bash
gh issue create --repo Kewton/Anvil \
  --title "[Bug] ${タイトル}" \
  --label "bug" \
  --body "${Issue本文}"
```
