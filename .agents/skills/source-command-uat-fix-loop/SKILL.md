---
name: "source-command-uat-fix-loop"
description: "UAT不合格→featureブランチ修正→再PR→再マージ→再UATの修正ループを自動化"
---

# source-command-uat-fix-loop

Use this skill when the user asks to run the migrated source command `uat-fix-loop`.
...
1. cargo fmt --all
2. cargo clippy --all-targets（警告0件であること）
3. cargo test（全テストパスであること）
4. 修正内容をコミット
5. git push
```
...
for each PR:
  # コンフリクト確認
  gh pr view <PR_NUM> --repo Kewton/Anvil --json mergeable
...
commandmatedev send <worktree-id> \
  ...
  解消後 cargo build && cargo test で確認し、git push --force-with-lease してください" \
  --auto-yes --duration 1h
