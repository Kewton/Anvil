---
name: "source-command-apply-review"
description: "アーキテクチャレビュー結果を設計方針書に反映"
---

# source-command-apply-review

Use this skill when the user asks to run the migrated source command `apply-review`.
...
```bash
cat dev-reports/issue/{issue_number}/architecture-review.md
```
...
```bash
cat dev-reports/issue/{issue_number}/design-policy.md
```
...
Rust固有の規約変更がある場合、該当セクションを更新します：

```rust
// 例: trait設計の改善
// Before: 巨大なtraitに全メソッドを集約
trait Provider {
    fn chat(&self, ...) -> ...;
    fn stream(&self, ...) -> ...;
    fn models(&self) -> ...;
...
`dev-reports/issue/{issue_number}/design-policy.md`（更新）
