---
name: "source-command-multi-stage-design-review"
description: "設計書の4段階レビュー（通常→整合性→影響分析→セキュリティ）と指摘対応を自動実行"
---

# source-command-multi-stage-design-review

Use this skill when the user asks to run the migrated source command `multi-stage-design-review`.
...
| ステージ | レビュアー | 理由 |
|---------|----------|------|
| Stage 1-2（通常・整合性） | **Codex opus**（サブエージェント） | 品質判断にOpus必要 |
...
> Codex サブエージェント（Agent tool）で代替実行してはならない。
...
> **⚠ 禁止事項（厳守）**:
> 1. このステージをCodexサブエージェント（Agent tool）で代替実行しないこと。必ず `commandmatedev send --agent codex` でCodexに送信すること。
> 2. Codexが生成した結果ファイルを Codex 側で再実行・上書きしないこと。Codex が findings 0件を返した場合も「問題なし」という正当な結果であり、件数が少ないことを理由に opus で再レビューすることを禁止する。
...
> **⚠ 上書き禁止**: Codexの結果ファイルが既に存在する場合、Codex側で再実行・上書きしてはならない。
...
> **⚠ 上書き禁止**: Stage 3-2 と同様、Codexの結果を Codex 側で再実行・上書きしてはならない。
