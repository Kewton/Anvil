---
name: "source-command-multi-stage-issue-review"
description: "Issue記載内容の多段階レビュー（通常→影響範囲）×2回と指摘対応を自動実行"
---

# source-command-multi-stage-issue-review

Use this skill when the user asks to run the migrated source command `multi-stage-issue-review`.
...
| ステージ | レビュアー | 理由 |
|---------|----------|------|
| Stage 1-4（1回目） | **Codex opus**（サブエージェント） | 品質判断にOpus必要 |
| Stage 5-8（2回目） | **Codex**（commandmatedev `--agent codex` 経由） | 異なるモデルによるクロスレビュー |
| 指摘反映（全ステージ） | sonnet（継承） | JSON→Issue更新のみ |

> **⚠ 禁止事項（厳守）**:
> 1. Stage 5-8 は必ず `commandmatedev send ... --agent codex` でCodexに委譲すること。Codex サブエージェント（Agent tool）で代替実行してはならない。
...
> **⚠ 禁止事項**: このステージをCodexサブエージェント（Agent tool）で代替実行しないこと。
