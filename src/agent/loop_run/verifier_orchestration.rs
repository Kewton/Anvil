...
use super::verifier_repair_shadow::{
    verifier_repair_action_payload_for_context, verifier_repair_action_space_payload_for_context,
};
...
pub(super) fn verifier_repair_pass_messages_with_runtime_capability(
    ...
    let repair_action_space =
        verifier_repair_action_space_payload_for_context(context, target_hint);
    let observed_expected_pairs = super::failure_packet::FailurePacket::from_repair_job(context)
    ...
    let api_contract = super::api_contract_expectation::api_contract_payload_value_from_request(
        active_request,
        &context.output_excerpt,
    );
    ...
    let payload = serde_json::json!({
        ...
        "repair_action_space": repair_action_space,
        "observed_expected_pairs": observed_expected_pairs,
        "selected_target": {
...
pub(super) fn apply_verifier_repair_pass_edit(
    ...
    let undo = super::repair_patch_executor::apply_validated_repair_edit(&edit).map_err(|err| {
        ValidationFailure::failed(format!("failed to apply {}: {err}", edit.relative_path))
    })?;
...
pub(super) fn record_controller_verifier_repair_edit(
    ...
    fingerprint: &str,
    target_hint: &RecoveryTargetHint,
    undo: super::repair_patch_executor::AppliedRepairPatchUndo,
) {
    ...
    if let Some(context) = agent.repair_job.as_mut() {
        ...
        context.pending_patch_undo = Some(super::repair_job::PendingRepairPatchUndo {
            key: key.clone(),
            undo,
        });
