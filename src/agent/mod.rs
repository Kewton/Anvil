...
pub fn tool_protocol_system_prompt(
    ...
    let base = concat!(
        ...
        "\n",
        "8. git.status — show working tree status (porcelain format):\n",
        "```ANVIL_TOOL\n",
        "{\"id\":\"call_010\",\"tool\":\"git.status\"}\n",
        "```\n",
        "\n",
        "9. git.diff — show changes in the working tree or between commits:\n",
        "```ANVIL_TOOL\n",
        "{\"id\":\"call_011\",\"tool\":\"git.diff\",\"path\":\"src/main.rs\",\"staged\":true}\n",
        "```\n",
        "Parameters: path (optional), staged (optional, bool), commit (optional, e.g. \"HEAD~3\"). If staged is true, commit is ignored.\n",
        "\n",
        "10. git.log — show commit log (oneline format):\n",
        "```ANVIL_TOOL\n",
        "{\"id\":\"call_012\",\"tool\":\"git.log\",\"count\":20,\"path\":\"src/\"}\n",
        "```\n",
        "Parameters: count (optional, 1-100, default 10), path (optional).\n",
        "\n",
        "11. agent.explore — launch a read-only sub-agent to explore the codebase:\n",
        "```ANVIL_TOOL\n",
        "{\"id\":\"call_008\",\"tool\":\"agent.explore\",\"prompt\":\"Investigate the module structure under src/\",\"scope\":\"./src\"}\n",
        "```\n",
        "The sub-agent can only use file.read, file.search, and git tools within the given scope directory.\n",
        "\n",
        "12. agent.plan — launch a read-only sub-agent to create an implementation plan:\n",
        "```ANVIL_TOOL\n",
    ...
    prompt.push_str(concat!(
        "\n\n## Git operations\n",
        "When working with Git, prefer the dedicated git tools (git.status, git.diff, git.log) over shell.exec for read-only queries.\n",
        "These dedicated tools are Safe/ReadOnly and require no approval.\n",
        "\n",
        "**Safe (auto-approved):**\n",
        "- git.status, git.diff, git.log (dedicated tools — preferred)\n",
        "- git status, git log, git diff, git branch, git show <ref>, git remote -v, git rev-parse (via shell.exec)\n",
        "\n",
