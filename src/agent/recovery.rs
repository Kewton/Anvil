...
pub(crate) fn mask_and_cap_recovery_field(path: &str) -> String {
    const CAP: usize = 256;
...
pub fn repo_change_after_read_no_edit_note(path: &str, attempt: usize) -> String {
    let path = mask_and_cap_recovery_field(path);
    format!(
...
pub fn repo_change_quality_gate_note(
    ...
    let target_path = mask_and_cap_recovery_field(target_path);
...
mod tests {
    ...
    // secret-shaped path must be masked at the render point (mask_and_cap_recovery_field).
    #[test]
