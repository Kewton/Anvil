...
### 2.8 Profile Selection Rule

`TaskContract` から task-type requirement profile を deterministic に選ぶ。
...
- `browser-runnable` + `html_app` -> `html_app`
- `browser-runnable` + `game-like interaction` -> `game`
- `code modification without new runtime` -> `refactor`

profile selection は fixed workflow ではなく、required evidence set の選択に限る。

### 2.9 Profile Confidence Model

profile selection には confidence を持たせる。
...
- high confidence なら selected profile をそのまま使う
- medium/low confidence なら fallback profile を残し、clarification 余地を確保する
...
### 3.11 Fallback Provenance

tool call / tool execution の provenance を追跡できるようにする。

- `provider_native`
- `normalized_fallback`
- `denied`

fallback provenance は audit と debug UI に残す。

### 3.12 Fallback Quality Penalty

fallback provenance が `normalized_fallback` の場合は quality penalty を課す。

- finalize 前に追加 verification を要求できる
- review evidence として単独では扱わない
- high-risk action の根拠には使わない
...
## 5. Plan / Execution Artifact Split

### 5.1 Goal

長時間タスクで plan と execution の責務を分ける。
...
### 5.3 Plan Artifact Lifecycle

plan artifact は次の状態を持てる。
...
execution artifact は plan artifact と紐付け可能にする。

### 5.4 Artifact Link Graph

artifact 間のリンクを追跡できるようにする。

- `plan_artifact -> execution_evidence`
- `execution_evidence -> review_finding`
- `review_finding -> final_response`

### 5.5 Post-Write Verification Hooks

write/edit 後に task profile に応じた safeguard hook を差し込めるようにする。
...
- targeted test hook
- verification evidence injection

hook は optional であり、task profile と permission state に応じて有効化する。

### 5.6 Safeguard Trigger Events

safeguard は event ベースで発火条件を持つ。
...
## 6. Message-Structured Prompt Layout
...
9. `system`
- plan artifact summary / execution safeguard bundle

### 6.3 Task-Class Execution Stance
...
### 6.5 Stance Priority Stack

stance が競合した場合の優先順位を固定する。
...
### 6.6 Stance Transition Audit

stance 変化は audit / debug UI で追えるようにする。
...
### 6.7 Working Transcript と Carryover Summary
...
### 6.8 Transcript Integrity Rules
...
- execution safeguard bundle availability

### 6.11 Permission Lifetime Policy
...
### 6.12 Approval Compression Policy

低リスク確認は圧縮できるようにする。

- 同種の低リスク approve は session allow へ畳める
- 危険操作は圧縮対象外
- compression の結果は permission snapshot と audit に残す

### 6.13 Permission Event Retention Policy
...
### 6.14 Execution Safeguard Bundle

task profile に応じて実行後 safeguard を束ねられるようにする。
...
- `autotest_policy`
- `watcher_policy`
- `verification_hook_policy`

### 6.15 Transcript Retention Policy
...
7. `assistant/tool`: compacted transcript

これで単一巨大 string よりかなり安定する。

### 6.18 Assistant Transition Note
...
## 7. Evidence Delta と Repeat Recovery
...
### 7.2 Repeat Recovery と Fail-Safe
...
### 7.3 Streaming Fallback Retry Rule
...
- `vibe-local` よりも plan/execution artifact, stance transition, safeguard hook が構造化される
- profile confidence / artifact link / approval compression / fallback quality penalty により `vibe-local` より説明可能で制御しやすい
