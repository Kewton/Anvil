...
| Stage 3-4（影響分析・セキュリティ） | **Codex**（commandmatedev経由） | 異なるモデルによるクロスレビュー |
...
Stage 3（影響分析）とStage 4（セキュリティ）は**Codex**に委譲し、異なるモデルによるクロスレビューで品質を向上させます。

#### 3-1. Codexへの影響分析レビュー依頼

commandmatedev CLIを使い、同一worktreeのCodexセッションにレビューを依頼します。
